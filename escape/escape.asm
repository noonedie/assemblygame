...
processMouseMove PROC
	pusha
	.if scene == 0 ;主菜单界面
		invoke clickPosition,  18,280,288,420
		.if eax == 1
			invoke AddMask, hInstance, @hDC, START_REPLCAE, 18,280,288,420
		.endif
		invoke clickPosition, 325,280,595,420
		.if  eax == 1
			invoke AddMask, hInstance, @hDC, HELP_REPLACE, 323,280,593,420
		.endif
	.elseif scene == 1
		.if death == 1;死亡界面的处理
			invoke clickPosition, 66, 281, 221, 371
			.if eax == 1;点击了“重新开始”
				invoke AddMask, hInstance, @hDC, RESTART_REPLCAE, 66, 281, 221, 371
			.endif
			invoke clickPosition, 225, 280, 380, 370
			.if eax == 1;点击了“返回主菜单”
				invoke AddMask, hInstance, @hDC,RETURN_REPLCAE, 225, 280, 380, 370
			.endif
		.endif	
	.elseif scene == 2;帮助界面
		invoke clickPosition, 370,340,516,385
		.if eax == 1
			invoke AddMask, hInstance, @hDC,DIRECT_REPLACE, 360,310,506,355
		.endif
	.endif
	popa
	ret
...
;********************************************************************
	...
	.else ;取出最高得分，统计最高得分
		...
		mov lMode, eax
		invoke TextOut,Dc, 230, 155, ADDR scoreInfo, len
		...
		.if readLength == 0 || eax < score
			invoke WriteFile,
				...
				NULL
			invoke SetFilePointer, fileHandle, 0, 0, FILE_BEGIN
			invoke TextOut,Dc, 230, 210, ADDR scoreInfo, len	
		.else
			invoke scoreTrans, highScore	
			invoke TextOut,Dc, 230, 210, ADDR scoreInfo, len	
		.endif
		pop eax
		invoke SetBkMode, Dc, lMode
	.endif
